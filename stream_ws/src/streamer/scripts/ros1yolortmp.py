...
class YOLORtmpStreamer:
    def __init__(self):
        ...
        # ROS 노드 초기화
        rospy.init_node('yolo_rtmp_streamer', anonymous=True)

        self.rtmp_url = rospy.get_param('~rtmp_url', 'rtmp://gaemibot.duckdns.org/LiveApp/rgb')
        self.frame_rate = rospy.get_param('~frame_rate', 15)
        self.preset = rospy.get_param('~preset', 'ultrafast')
        self.debug_mode = rospy.get_param('~debug_mode', False)

        image_topic = rospy.get_param('~image_topic', '/yolo/color/compressed')
        # 토픽 구독
    ...
    def _check_memory_usage(self, event):
        ...
        rospy.loginfo(f"현재 메모리 사용량 : {memory_usage : 0.2f} ")
...
def main():
    streamer = YOLORtmpStreamer()
    streamer.start_timer()
    try:
        rospy.spin()
